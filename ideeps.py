...
# Added
def padding_sequence(seq, max_len=375, shift=0, repkey='N'):
    if shift >= 0: # shift right
        seq = repkey * shift + seq 
    else: # shift left
        seq = seq[abs(shift):]
...
def get_RNA_seq_concolutional_array(seq, motif_len = 10):
    ...
    for i, val in enumerate(seq):
        # i = i + motif_len-1 # changed: motif_len -> half_len
        i = i + half_len # changed: motif_len - 1 -> half_len
        if val not in 'ACGT':
            new_array[i] = np.array([0.25]*4)
            continue
...
def get_RNA_structure_concolutional_array(seq, fw, structure = None, motif_len = 10):
    ...
    # Added
    struc_en = padding_sequence(struc_en)
...
def get_cnn_network():
    ...
    seq_model = set_cnn_model(4, 385) # 111 -> 385
    struct_model = set_cnn_model(6, 385) # 111 -> 385
    #pdb.set_trace()
    model = Sequential()
...
def get_cnn_network_old():
    ...
    model = Sequential()
    model.add(Convolution1D(input_dim=4,input_length=385, # 111 -> 385
                            nb_filter=nbfilter,
                            filter_length=10,
                            border_mode="valid",
                            #activation="relu",
...
def get_struct_network():
    ...
    model = Sequential()
    model.add(Convolution1D(input_dim=6,input_length=385, # 111 -> 385
                            nb_filter=nbfilter,
                            filter_length=10,
                            border_mode="valid",
                            #activation="relu",
...
def load_data_file(inputfile, seq = True, onlytest = False):
    ...
    if seq: 
        ...
        tmp.append(seq_onehot)
        data["seq"] = tmp # 0: base, 1: structure
        data["structure"] = structure
    if onlytest:
        data["Y"] = []
    else:
...
def run_network_new(model, total_hid, training, y, validation, val_y, batch_size=50, nb_epoch=30, verbose=0):
    model.add(Dense(2, input_shape=(total_hid,)))
    model.add(Activation('softmax'))
    ...
    earlystopper = EarlyStopping(monitor='val_loss', patience=5, verbose=verbose)

    model.fit(training, y, batch_size=batch_size, nb_epoch=nb_epoch, verbose=verbose, validation_data=(validation, val_y), callbacks=[earlystopper])

    return model    
    
def train_ideeps(data_file, model_dir, batch_size= 50, nb_epoch = 30, verbose=0):
    ...
    ## for now - uncomment these lines to train models
    # total_hid = seq_hid + struct_hid
    # model = run_network_new(seq_net, total_hid, cnn_train, y, validation = cnn_validation, val_y = val_y, batch_size=batch_size, nb_epoch = nb_epoch, verbose=verbose)
...
def run_ideeps(parser):
    ...
    if train:
        print 'model training'
        train_ideeps(data_file, model_dir, batch_size= batch_size, nb_epoch = n_epochs, verbose=verbose)
    else:
...
def parse_arguments(parser):
    ...
    parser.add_argument('--verbose', type=int, default=0, help='Whether to print training procedure. 0: suppress, 1: print')
